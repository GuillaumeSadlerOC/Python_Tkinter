...
class Empty():
    def __init__(
        ...
        # Widgets row 0
        self.w_empty_img = None
        self.w_empty_button = None
    ...
    def row_0(self, action=None):
        ...
        if action == "construct":
            ...
            if self.w_empty_img is None:

                img = Image.open(
                    "frontend/images/empty.png"
                )
                imgResize = img.resize((300, 300), Image.ANTIALIAS)
                imgTkinter = ImageTk.PhotoImage(imgResize)

                self.w_empty_img = Label(
                    self.grid.col_frames[0][1],
                    image=imgTkinter,
                    borderwidth=5,
                    bg="#ffffff"
                )
                self.w_empty_img.image = imgTkinter
                self.w_empty_img.pack(fill="both", expand=True)

            # -- COLUMN 3/3 : ERROR 404 IMG -- #
            if self.w_empty_button is None:

                self.w_empty_button = Button(
                    self.grid.col_frames[0][1],
                    text=txt.get("empty_button"),
                    fg="#ffffff",
                    bg="#6B7379",
                ...
                self.w_empty_button.pack(expand=True)

        elif action == "refresh":

            self.w_empty_img.pack_forget()
            self.w_empty_button.pack_forget()
