...
class SearchResult():
    ...
    def fill(self, **kwargs):
        ...
        for key, value in kwargs.items():
            ...
            elif key == "search_action":
                self.search_action = value
        ...
        if self.search_action == "nutriscore":
            # Get products
            self.products = self.session.dbmanager.db_product.read(
                    action="nutriscore",
                    product_nutriscore=self.search_value
                )
        elif self.search_action == "cat_id":
            # Get products

            self.products = self.session.dbmanager.db_product.read(
                    action="cat_id",
                    product_cat_id=self.search_value
                )
        elif self.search_action == "key":
            # Get products
            self.products = self.session.dbmanager.db_product.read(
                    action="name",
                    product_name=self.search_value
                )
        else:
            # Get products
            self.products = self.session.dbmanager.db_product.read(
        ...
        self.page = 0
        self.pages = int(self.products_count / 6)
        self.paging_products = self.products[0:6]
    ...
    def paging(self, p_action=None):
        ...
        self.paging_products = self.products[self.p_start:self.p_stop]
    ...
    def row_1(self, action=None):
        ...
        if action == "construct":
            ...
            count = 0
            for product in self.paging_products[0:3]:
    ...
    def row_2(self, action=None):
        ...
        if action == "construct":
            ...
            count = 0
            for product in self.paging_products[3:6]:
    ...
    def row_3(self, action=None):
        ...
        if action == "construct":
            ...
            if self.w_submit_button is None:
                ...
                self.w_submit_button.pack(fill="x", expand=True)
